...
#' @param all Indicates to search globally. Input all=TRUE to fetch all country codes. Defaults to FALSE.
#' @return The sum of \code{x} and \code{y}.
#' @examples Country_Codes(level="UN_subregion", region=c("Western_Europe", "Eastern_Europe"))
#' 
Country_Codes <- function(level="Realm", region="Palearctic", all=FALSE){
    geo_area <- c() # Initialise empty vector
    
    if(all==TRUE){
        geo_area <- Country_Code_CSV$Country_Code
    }
